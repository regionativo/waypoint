...
export async function importFromJSON(jsonArray) {
  ...
  for (const item of jsonArray) {
    const { url, title, tags = [], speedDial = null } = item;
    if (!url) continue;
    ...
    if (existing) {
      ...
      if (mergedTags.length > existing.tags.length) {
        await saveBookmark({ url, title: existing.title, tags: mergedTags, favIconUrl: existing.favIconUrl, speedDialSlot: speedDial });
        merged++;
      } else {
        skipped++;
      }
    } else {
      await saveBookmark({ url, title: title || url, tags: normalizedTags, speedDialSlot: speedDial });
      imported++;
    }
  }
